...
void VoxelArrangement::computeFeaturesPtNormal(const vector<Point> &points, const vector<Point> &pointOfViews,
                                               const vector<Vector> &normals, bool verbose) {
    // Initialize the features
    _features = vector<vector<vector<vector<double>>>>(_width,
                        vector<vector<vector<double>>>(_height,
                                vector<vector<double>>(_depth,
                                        vector<double>(4, 0.)))); // Normal + Visibility
    ...
    for(int i=0; i < points.size(); i++) {
        ...
        vector<double> &curFeature = _features[get<0>(cellIdx)][get<1>(cellIdx)][get<2>(cellIdx)];
...
#pragma omp critical
            {
                _features[get<0>(voxel)][get<1>(voxel)][get<2>(voxel)][3]++;
...
int splitArrangementInVoxelsPure(vector<facesLabelName> &labeledShapes,
                                    const vector<Point> &pointOfViews,
                                    const vector<Point> &pointCloud,
                                    ...
                                    bool withRichFeatures, bool verbose)
{
    ...
    for(int i=0; i < bboxes.size(); i++)
    {
        ...
        // We compute the features
        fullArrangement.computeFeaturesPtNormal(pointCloud, pointOfViews, normals, verbose);
